...
const runTests = function ( tester, funcsWithArgs1, name ) {
	...
	const singleAssertions = require('./helpers/single-assertions.js')( plab ),
	...
	const increment = function ( indices ) {

		const two = indices.two, one = indices.one;
	...
	function iterate ( label = '', func1Indx = 0, arg1Indx = 0, event1Indx = 0, func2Indx = 0, arg2Indx = 0, event2Indx = 0 ) {
		...
		const runAssert = function ( result, assertNum, shouldSkip ) {
		// console.log('called');
		...
		const result = { playback: null, arg2s: [] };

		var firstCalled = false;
		const afterFirstEvent = function () {
	...
	if ( typeof process.argv[2] === 'string' ) {
		...
		if ( arg === 'all' ) {
			...
			if ( indx < funcsWithArgs1.length && obj ) {
				...
				const name = obj.func + '(' + JSON.stringify( obj.args[0] ) + ')';
